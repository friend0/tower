...
sim_message_keypress = 6  # a key was pressed while the focus was on a page (aux[0]=key aux[1]=ctrl and shift key
# state)
...
sim_message_eventcallback_rmlposition = 0x12b  # the command simRMLPosition was called. The appropriate
#  plugin should handle the call
...
simx_headeroffset_server_time = 11  # 1 simxInt. Generated by the server when a reply is generated. The server
# timestamp
...
simx_opmode_streaming = 0x020000  # sends command as one chunk. Command will be stored on the server and always
# executed
