...
from world_engine.world.vehicles.vehicle import Quadrotor, Coordinate, PixelPair
...
class Map_Process(Map, multiprocessing.Process):
    ...
    def run(self, context=None, worker_ip=None, worker_port=5555):
        ...
        # Get vehicle position updates from dynamic models
        model = context.socket(zmq.PAIR)
        model.connect("tcp://localhost:%s" % settings['MODEL_PORT'])
